...
Non-interactive. Brings a node installed by an earlier version up to the current layout **without touching** the RNS config, lxmd config, `notifier.conf` values or the APNs keys: strips world-read access from `/var/lib/reticulum`, adds `UMask=0027` to the rnsd/lxmd units, migrates a stale `messagestore` path, re-applies the notifier's read-only ACL, fixes ownership of keys/state, and refreshes the notifier code + service definition (the key drop-ins are preserved). Run `checkHealth.sh` afterwards; if it reports something fixable, `--fix` is the intended remedy.
